...
# 	22. Optional SUBOPTIMALTHRESHOLD (default is 40) to compare first vs second hit (AS vs XS tags), this is a percentage value (int).
...
#R1="/opt/NGS/data/Insulator/20130802_Stupka_MiSeq_MV1/SA55_S1_L001_R1_001.fastq.gz" ;
#R2="/opt/NGS/data/Insulator/20130802_Stupka_MiSeq_MV1/SA55_S1_L001_R2_001.fastq.gz" ;
R1="/opt/NGS/data/Insulator/20130802_Stupka_MiSeq_MV1/SA55_S1_L001_R1_001.fastq.40ktest.gz" ;
R2="/opt/NGS/data/Insulator/20130802_Stupka_MiSeq_MV1/SA55_S1_L001_R2_001.fastq.40ktest.gz" ;
POOLNAME="test_mv1";
PATIENT="Safety";
GENOME="/opt/genome/mouse/mm9/index/bwa_7/mm9.fa";
DISEASE="Insulator";
BARCODE_LTR="/opt/applications/scripts/isatk/testing/validation/barcode.hic.mv.ltr.small.list" ;
BARCODE_LC="/opt/applications/scripts/isatk/testing/validation/barcode.hic.mv.lc.small.list" ;
...
DBHOSTID="local"
DBTARGETSCHEMA="sequence_insulator"
DBTARGETTABLE="test_mv1"
...
if [ ${LEFTSPACE} -lt 15000000 ]; then 
    ...
    *****************************************************************
	...
	"  > ${LOGF};
	date >> ${LOGF}
	/opt/applications/scripts/isatk/pipeline/illumina/./IlluminaMiSeq.pipeline.gemini.sh ${DISEASE} ${PATIENT} ${NGSWORKINGPATH} ${R1} ${R2} ${POOLNAME} ${BARCODE_LTR} ${BARCODE_LC} ${GENOME} ${TMPDIR} ${ASSOCIATIONFILE} ${DBHOSTID} ${DBTARGETSCHEMA} ${DBTARGETTABLE} ${PHIXGENOME} ${LVGENOME} ${CONTAMINANTDB} ${MAXTHREADS} ${GATKREFGENOME} ${CIGARGENOMEID} ${STRINGETREGIONSBED} ${SUBOPTIMALTHRESHOLD} 2>&1 #>> ${LOGF} ;
	echo "
