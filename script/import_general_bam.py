...
#parser.add_argument('--tmpfolder', dest="tmpfolder", help="The temporary folder in which the app can write data; in this folder you must have write permissions.", action="store", default="/tmp")
...
def getRegions(regions_str):
	...
	for reg in regions_str.split(';'):
		chr = None
		if reg.strip().split(':')[0].strip().isdigit() or reg.strip().split(':')[0].strip() in ['X', 'Y', 'M']:
		...
		if locusRead == 'read1': # locus is in r1
			...
			for k, v in product.iteritems():
				...
				if v.has_key(locus):
					...
					is_array = [k, v[locus]['chr'], v[locus]['start'], prod_end, v[locus]['strand'], 'associationID', 'NULL', 'poolID', ]
			...
			locus = 'r'
			for k, v in product.iteritems():
				is_array = [k, v[locus]['chr'], v[locus]['start'], v[locus]['end'], v[locus]['strand'], 'associationID', 'NULL', 'poolID', ]
				for x in readorder: # first the locus read
					is_array.append( str(v[locus][x]) )
				for x in readorder: 
					is_array.append( 'NULL' )
...
def main():
	...
	for i in range(nprocs):
		p = multiprocessing.Process(target=worker, args=(regions[chunksize * i:chunksize * (i + 1)], bam, args.singleEnd, args.locusRead, out_q, )) 
		procs.append(p)
		p.start()
