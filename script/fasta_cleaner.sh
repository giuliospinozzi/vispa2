...
INPUTFASTA="${FASTA}"
# identify LTR
flexbar2.5 --reads ${INPUTFASTA} -f i1.8 -a ${LTR} --threads ${MAXTHREADS} -ae LEFT -at 3.6 -ai -4 -ao 22 -m 15 -q 1 --stdout-reads --removal-tags --max-uncalled 800 | grep Flexbar | cut -d' ' -f 1 | sed 's/>//g' | sed 's/_Flexbar_removal//g' | gzip -f > ${DDIR}/${BNAME_FASTA}.validLTR.list.gz
...
fasta_to_fastq -q 9 ${FASTA} | fastq_to_fasta -n | faextract_pureheader <( zcat ${DDIR}/${BNAME_FASTA}.validLTR.list.gz ) | gzip -f > ${DDIR}/${BNAME_FASTA}.validLTR.fa.gz
# trim LTR
...
# fastqc -o ${OUTDIR_QUAL} --contaminants ${SEQCONTAM} -t ${MAXTHREADS} -f bam ${BASEDIR}/${DISEASE}_${PATIENT}_${POOL}.LTR${b}.LC${k}.noLTRLC.${PLASMID}.merge.bam
samtools view ${DDIR}/${BNAME_FASTA}.${PLASMID}.sorted.bam | cut -f1 | sort | uniq | gzip -f > ${DDIR}/${BNAME_FASTA}.${PLASMID}.list.gz
rm ${DDIR}/${BNAME_FASTA}.*.sorted.bam
PLASMID_MAPPING_READS=`zcat ${DDIR}/${BNAME_FASTA}.${PLASMID}.list.gz | wc -l  | cut -d' ' -f1 `
# extract only NO PHIX reads
zcat ${INPUTFASTA} | fareverseextract_pureheader <( zcat ${DDIR}/${BNAME_FASTA}.${PLASMID}.list.gz ) | gzip -f > ${DDIR}/${BNAME_FASTA}.trimLTR.trimLC.noplasmid.fa.gz
