...
def parseBEDfile(inbedfile):
	...
	with open(inbedfile, 'rb') as inf:
		...
		for row in reader:
			...
			if row[5] == '+':
				bedarray.append( (chr, row[1], row[3], row[5], row[4], row[6]) )
			else:
				bedarray.append( (chr, row[2], row[3], row[5], row[4], row[6]) )
...
def createTargetTable(dbschema, dbtable):
	...
	query = """
		CREATE TABLE IF NOT EXISTS `%s`.`%s` (
		  ...
		  `sequence_raw` varchar(500) DEFAULT NULL,
		  `sequence_trimmed` varchar(1000) DEFAULT NULL,
		  PRIMARY KEY (`header`),
...
if args.seqfileraw is not None and args.seqfiletrimmed is not None:
	...
	query_import = """
		INSERT INTO `%s`.`%s` (`group_name`, `n_LAM`, `pool`, `tag`, `sample`, `tissue`, `treatment`, `enzyme`, `complete_name`, `header`, `chr`, `integration_locus`, `strand`, `score`, `label`, `vector`, `sequence_raw`, `sequence_trimmed`) 
		VALUES 
	...
	for (chr, locus, header, strand, score, label) in beddata:
	...
	for elems in range(0, len(beddata), step):
		query_import = """
			INSERT INTO `%s`.`%s` (`group_name`, `n_LAM`, `pool`, `tag`, `sample`, `tissue`, `treatment`, `enzyme`, `complete_name`, `header`, `chr`, `integration_locus`, `strand`, `score`, `label`, `vector`) 
			VALUES 
			""" %(args.dbschema, args.dbtable)
		# write array of data in this order:: patient, lamid, pool, tag, sample, tissue, timepoint, enzyme, lam_name, header, chr, is, strand, score
		for (chr, locus, header, strand, score, label) in beddata[bedstart:bedend]:
			query_import += " ('" + "', '".join( str(x) for x in  
				[args.patient, 
				dict_asso[args.tag]["lam_id"], 
				args.pool, 
